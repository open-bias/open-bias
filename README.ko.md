...
v0.3.0 — 베타 단계입니다. 프록시 레이어, judge와 NeMo 엔진, 규칙 컴파일러, 리플레이/개선 도구, OpenTelemetry 트레이싱까지 모두 동작합니다. 나머지 두 엔진(FSM, LLM)은 아직 실험 단계입니다. 설정 없이 바로 뜨는 기본 구성과 선택적인 YAML 설정도 이미 준비되어 있습니다.
